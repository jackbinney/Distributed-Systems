...
public class BackupServer1 implements ServerInterface {
    // Boolean variable to monitor whether this server has become the primary server
	Boolean primary;
    // Keeps track of what port number this uses
	int port;
    // Stores all orders within the system
	Map<String, ArrayList<String[]>> orders = new HashMap<String, ArrayList<String[]>>();
    // Keeps track of what other servers should be in the system
	String[][] otherServers = {{"localhost", "main"}, {"localhost", "backup2"}};
	
    public BackupServer1(int port) {
        // On initialisation, this server shouldn't be a primary
    	primary = false;
        // Set the port
    	this.port = port;
